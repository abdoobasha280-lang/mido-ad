...
def start(message):
    ...
    if check_sub(message.from_user.id):
        ...
        markup = types.InlineKeyboardMarkup(row_width=1)
        markup.add(types.InlineKeyboardButton("قناة البوت 📢", url=f"https://t.me/{CHANNEL_ID[1:]}"),
...
def callback_handler(call):
    ...
    if call.data == "back_home":
        ...
        bot.edit_message_text("الخدمات الإضافية:", cid, mid, reply_markup=extra_markup())
        ...
        if check_sub(call.from_user.id): start(call.message)
        else: bot.answer_callback_query(call.id, "❌ لسه مشتركتش!", show_alert=True)
