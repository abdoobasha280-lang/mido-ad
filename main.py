...
# --- [ ملاحظة: تم حذف إعدادات البروكسي ليعمل على Railway بسلاسة ] ---
...
def extra_markup():
    ...
    markup.add(
        ...
        types.InlineKeyboardButton("📧 بريد مؤقت", callback_data="gen_email"),
...
# --- [ وظائف الخدمات الإضافية الجديدة ] ---
...
def create_random_temp_email(chat_id):
    try:
        ...
        if data.get('success'):
            ...
            bot.send_message(chat_id, f"📥 بريدك المؤقت الجديد:\n`{email}`", parse_mode="Markdown", reply_markup=extra_markup())
        else:
            bot.send_message(chat_id, "❌ خطأ في توليد البريد.")
    except:
        bot.send_message(chat_id, "❌ السيرفر لا يستجيب حالياً.")
...
def process_wallet_check(message):
    ...
    try:
        ...
        if status == "SUCCESS":
            bot.send_message(chat_id, f"✅ الرقم {number} مسجل في محفظة كاش بنجاح.", reply_markup=extra_markup())
        else:
            bot.send_message(chat_id, f"❌ الرقم {number} غير مسجل في أي محفظة.", reply_markup=extra_markup())
    except:
        bot.send_message(chat_id, "❌ حدث خطأ أثناء الفحص.")

def process_tiktok_info(message):
    username = message.text.strip()
    chat_id = message.chat.id
    bot.send_message(chat_id, "⏳ جاري جلب البيانات...")
    try:
        response = requests.get(f"{TIKTOK_API_URL}{username}")
        data = response.json()
        if 'error' in data:
            bot.send_message(chat_id, "❌ المستخدم غير موجود أو اليوزر خطأ.")
            return
        info = f"👤 الاسم: {data.get('nickname')}\n🆔 اليوزر: @{username}\n👥 المتابعين: {data.get('followers')}\n❤️ القلوب: {data.get('hearts')}"
        pic = data.get('profile_picture')
        bot.send_photo(chat_id, pic, caption=info, reply_markup=extra_markup())
    except:
        bot.send_message(chat_id, "❌ تعذر الوصول لبيانات تيك توك.")
...
def check_orange_balance(chat_id, phone):
    ...
    try:
        ...
        if 'CreditBalance' in res:
            bot.edit_message_text(f"💰 رصيدك في أورانج للرقم {phone} هو: {res['CreditBalance']} جنيه.", chat_id, loading.message_id, reply_markup=orange_markup())
        else: bot.edit_message_text("❌ لم يتم العثور على رصيد.", chat_id, loading.message_id)
    except: bot.edit_message_text("❌ خطأ في السيرفر.", chat_id, loading.message_id)

def run_fawazeer(chat_id, number, password):
    loading = bot.send_message(chat_id, "⏳ جاري الحل...")
    try:
...
def run_500mb(chat_id, number, password):
    loading = bot.send_message(chat_id, "⏳ جاري الطلب...")
    try:
        session = requests.Session()
        headers = {'User-Agent': "okhttp/4.10.0", 'Content-Type': "application/json"}
...
# --- [ منطق رسم الصور Nano Banana ] ---
def process_draw(message):
    prompt = message.text
    msg = bot.reply_to(message, "🎨 **جاري رسم خيالك باستخدام Nano Banana...**")
    try:
        ...
        bot.send_photo(message.chat.id, image_url, caption=f"✨ **تم التوليد بواسطة Nano Banana**\n\n📝 الوصف: `{prompt}`", reply_to_message_id=message.message_id, reply_markup=user_main_markup())
        bot.delete_message(message.chat.id, msg.message_id)
    except: bot.edit_message_text("يا أخويا السيرفر عليه ضغط، جرب كمان ثواني.", message.chat.id, msg.message_id)
...
def start(message):
    ...
    if message.from_user.id == ADMIN_ID:
        bot.send_message(message.chat.id, "🛠️ لوحة تحكم الأدمن:", reply_markup=admin_markup()); return
    if get_bot_status() == 0:
        bot.send_message(message.chat.id, "⚠️ البوت في وضع الصيانة."); return 
    bot.send_message(message.chat.id, f"🚀 أهلاً بك في {BOT_NAME}\nاستخدم الخدمات المجانية والاتصالات:", reply_markup=user_main_markup())

@bot.callback_query_handler(func=lambda call: True)
def callback_handler(call):
    cid, mid = call.message.chat.id, call.message.message_id
    if call.data == "back_home": bot.edit_message_text("القائمة الرئيسية:", cid, mid, reply_markup=user_main_markup())
    elif call.data == "vf_menu": bot.edit_message_text("🔴 خدمات فودافون:", cid, mid, reply_markup=vf_menu_markup())
    elif call.data == "orange_menu": bot.edit_message_text("🍊 خدمات أورانج:", cid, mid, reply_markup=orange_markup())
    elif call.data == "extra_menu": bot.edit_message_text("🛠️ خدمات إضافية متكاملة:", cid, mid, reply_markup=extra_markup())
    elif call.data == "gen_email": create_random_temp_email(cid)
    elif call.data == "check_wallet":
        msg = bot.send_message(cid, "📱 ارسل رقم الهاتف لفحص المحفظة:")
        ...
        msg = bot.send_message(cid, "📱 ارسل رقم فودافون:")
        bot.register_next_step_handler(msg, get_phone_step, "vf_flex")
    elif call.data == "check_balance":
        msg = bot.send_message(cid, "💰 ارسل رقم أورانج:")
        ...
        try:
            ...
            bot.edit_message_text(f"🕌 مواقيت الصلاة بتوقيت القاهرة:\n\nالفجر: {d['Fajr']}\nالظهر: {d['Dhuhr']}\nالعصر: {d['Asr']}\nالمغرب: {d['Maghrib']}\nالعشاء: {d['Isha']}", cid, mid, reply_markup=extra_markup())
        except: bot.answer_callback_query(call.id, "خطأ في السيرفر.")
    elif call.data == "draw_ai":
        msg = bot.send_message(cid, "🎨 ارسل وصف الصورة بالإنجليزي لرسمها:")
        ...
        msg = bot.send_message(cid, "📣 ارسل رسالة الإذاعة:"); bot.register_next_step_handler(msg, do_broadcast)
...
if __name__ == "__main__":
    print(f"{BOT_NAME} is Running with New Services...")
