...
# --- [ إعدادات البروكسي - لضمان العمل على السيرفرات ] ---
...
def check_orange_balance(chat_id, phone):
    ...
    try:
        ...
        if 'CreditBalance' in res:
            ...
            bot.edit_message_text("❌ عذراً، لم نتمكن من جلب الرصيد.", chat_id, loading.message_id)
...
def process_draw(message):
    ...
    try:
        ...
        bot.edit_message_text("يا أخويا السيرفر عليه ضغط، جرب كمان ثواني.", message.chat.id, msg.message_id)
...
def start(message):
    ...
    if message.from_user.id == ADMIN_ID:
        ...
        return
    if get_bot_status() == 0:
        bot.send_message(message.chat.id, "⚠️ البوت في حالة صيانة حالياً، عد لاحقاً.")
        return 
    bot.send_message(message.chat.id, f"🌟 أهلاً بك في {BOT_NAME}\nاستخدم Nano Banana للرسم أو خدمات الاتصالات:", reply_markup=user_main_markup())
...
def callback_handler(call):
    ...
    if call.data == "back_home":
        ...
        try:
            ...
            res = f"🕌 مواقيت الصلاة بتوقيت القاهرة:\n\nالفجر: {d['Fajr']}\nالظهر: {d['Dhuhr']}\nالعصر: {d['Asr']}\nالمغرب: {d['Maghrib']}\nالعشاء: {d['Isha']}"
        ...
        msg = bot.send_message(cid, "🎨 أرسل وصف الصورة (بالإنجليزي) لرسمها بتقنية Nano Banana:")
        ...
        msg = bot.send_message(cid, "📣 أرسل رسالة الإذاعة لجميع المستخدمين:")
...
if __name__ == "__main__":
    print(f"{BOT_NAME} is Online with Nano Banana Support...")
